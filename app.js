(async()=>{
  ...
  const loadScript=async(src,retries=2)=>{
    ...
    for(let i=0;i<=retries;i++){
      try{await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=`${src}?v=${V}`;s.onload=resolve;s.onerror=()=>{s.remove();reject(new Error(src))};document.head.appendChild(s)});return}catch(e){last=e;if(i<retries)await sleep(250*(i+1))}
    }
    throw last;
  };
  const fetchText=async(url,retries=2)=>{
    let last;
    for(let i=0;i<=retries;i++){
      try{const r=await fetch(`${url}?v=${V}`,{cache:'no-store'});if(!r.ok)throw new Error(`${url}:${r.status}`);return await r.text()}catch(e){last=e;if(i<retries)await sleep(250*(i+1))}
    }
    throw last;
  };
